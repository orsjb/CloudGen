 ...
 */
public class MOEARunner3_Select {
    ...
    public static void main(String[] args) throws IOException {
        ...
        DeciderMOEAProblem theProblem = new DeciderMOEAProblem(5, "data/Training") {
            ...
            public double[] evaluate(List<Number[][]> outputData, Decider d) {
                ...
                if(outputData != null) {
                    ...
                    results[0] = -1 * metric3.getMetric(outputData)[0];
                    SteadyRhythms metric4 = new SteadyRhythms();
                    results[1] = -1 * metric4.getMetric(outputData)[0];
                    DeciderSimulationStats metric5 = new DeciderSimulationStats();
                    double[] stats = metric5.getMetric(outputData);
                    results[2] = -1 * stats[8]; //entropy
                    results[3] = -1 * stats[1]; //number of nodes visited
                    EveryoneDoingSomething metric6 = new EveryoneDoingSomething();
                    results[4] = -1 * metric6.getMetric(outputData)[0];
                }
        ...
        metrics.write("Name,generation,variance,steady,entropy,total_nodes,everyone_busy\n");
        int steps = 20000;
        ...
        for(int i = 0; i < steps; i++) {
            ...
            if (i % writeInterval == 0) {
                ...
                System.out.println("Step NSGAII " + i);
            }
        }
